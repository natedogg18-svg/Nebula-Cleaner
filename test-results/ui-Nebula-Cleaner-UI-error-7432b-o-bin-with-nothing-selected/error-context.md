...
- Name: ui.spec.js >> Nebula Cleaner UI >> error when moving to bin with nothing selected
- Location: tests\ui.spec.js:152:3
...
Call log:
  - waiting for locator('button:has-text("Move to Bin")')
    - locator resolved to 3 elements. Proceeding with the first one: <button class="btn btn-sm btn-danger" onclick="sendSelectedToBin('duplicates')">🗑 Move to Bin</button>
