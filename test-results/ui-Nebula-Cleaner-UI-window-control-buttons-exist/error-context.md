...
- Name: ui.spec.js >> Nebula Cleaner UI >> window control buttons exist
- Location: tests\ui.spec.js:161:3
...
Locator: locator('.ctrl-btn.close')
Expected: visible
Error: strict mode violation: locator('.ctrl-btn.close') resolved to 2 elements:
    1) <button title="Close" class="ctrl-btn close" onclick="window.nebula.close()">×</button> aka getByRole('button', { name: '×' })
    2) <button class="ctrl-btn close" onclick="closeModal()">×</button> aka locator('#move-modal').getByText('×')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('.ctrl-btn.close')
